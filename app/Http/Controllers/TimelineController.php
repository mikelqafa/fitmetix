...
class TimelineController extends AppBaseController
{
    ...
    public function showTimeline($username)
    {
        ...
        $user_post = '';
        if ($timeline == null) { return redirect('/'); }
        ...
        $prev_month = $cMonth-1;
        $next_month = $cMonth+1;
        if ($prev_month == 0 ) { $prev_month = 12;$prev_year = $cYear - 1;  }
        if ($next_month == 13 ) { $next_month = 1; $next_year = $cYear + 1; }
        $event_date = array();
        $event_list = '';
        ...
        if ($timeline->type == 'user') {
            ...
            $user_events = $user->events()->whereMonth('start_date', '=', $cMonth)->whereYear('start_date', '=', $cYear)->get();
            $guest_events = $user->getEvents();
            $follow_user_status = DB::table('followers')->where('follower_id', '=', Auth::user()->id)->where('leader_id', '=', $user->id)->first();
            foreach($user_events as $ue) {
                $event_list .= '<div class="day-event panel panel-default" date-month="'.date('n', strtotime($ue->start_date)).'" date-day="'.date('j', strtotime($ue->start_date)).'"><div class="panel-heading"><a href="'.url($ue->timeline->username).'"><b class="panel-title">'.$ue->timeline->name.'</b></a></div><div><p>'.str_limit(strip_tags($ue->timeline->about), 100, '...').'</p><p>Location: <b>'.$ue->location.'</b></p></div></div>';
                $event_date[] = date('d', strtotime($ue->start_date));
            }
    ...
    public function showEvents(Request $request) {
        ...
        $theme = Theme::uses(Setting::get('current_theme', 'default'))->layout('default');
        $timeline = Timeline::where('username', Auth::user()->username)->first();
        $posts = Event::where('type', '=', 'public')->where( 'end_date' , '>', date('Y-m-d H:i', strtotime(Carbon::now())))->with('users')->with('timeline')->Paginate(Setting::get('items_page'));
        $trending_tags = trendingTags();
        $suggested_users = suggestedUsers();
        if ($request->ajax) {
            $responseHtml = '';
        ...
        $next_page_url = url('ajax/get-more-feed?page=2&ajax=true&hashtag='.$request->hashtag.'&username='.Auth::user()->username);
        $theme->setTitle(' '.Setting::get('title_seperator').' '.Setting::get('site_title').' '.Setting::get('title_seperator').' '.Setting::get('site_tagline'));

        return $theme->scope('event', compact('timeline', 'posts', 'next_page_url', 'trending_tags', 'suggested_users', 'active_announcement', 'suggested_groups', 'suggested_pages', 'mode', 'user_post'))
            ->render();
    }
    ...
    public function joiningEvent(Request $request)
    {
        ...
        if($event->gender != 'all') {
            if($event->gender != Auth::user()->gender) {
                return response()->json(['status' => '200', 'joined' => false, 'message' => 'successfully unjoined- Please provide your gender in your profile!']);
            }
        }
        $users = $event->users()->get();
        if (!$event->users->contains(Auth::user()->id)) {
            $getThis = $event->users()->where('user_id', Auth::user()->id)->first();
            if($event->user_limit == 0) {
            ...
            } elseif( $users->count() <= $event->user_limit) {
                if($event->price <= 0.01) {
                    $event->users()->attach(Auth::user()->id);
                    $pivoty = $event->users()->where('users.id', Auth::user()->id)->first();
                    $pivoty->pivot->status = 1;
                ...
                return response()->json(['status' => '200', 'joined' => false, 'message' => 'Maximum User Limit Reached!']);
            }
        } else {
            if($event->price <= 0.01) {
                $event->users()->detach([Auth::user()->id]);
            } else {
                $pivoty = $event->users()->where('users.id', Auth::user()->id)->where('status', 1)->first();
                if(!is_null($pivoty)) {
                    $event->users()->newPivotStatement()->where('user_id', Auth::user()->id)->where('status', 1)->update(['status' => 2]);
/*                    foreach ($users as $user) {
                        if ($user->id != Auth::user()->id) {
                            //Notify the user for page like
                    ...
                    $event->users()->newPivotStatement()->where('user_id', Auth::user()->id)->where('status', 2)->update(['status' => 1]);
                    foreach ($users as $user) {
                        if ($user->id != Auth::user()->id) {
                            //Notify the user for event join
                            Notification::create(['user_id' => $user->id, 'timeline_id' => $request->timeline_id, 'notified_by' => Auth::user()->id, 'description' => Auth::user()->name.' '.trans('common.attending_your_event'), 'type' => 'join_event']);
    ...
    protected function validateEventPage(array $data)
    {
        return Validator::make($data, [
            ...
            'user_limit'  => 'required|numeric|min:0',
            'gender'      => 'required|in:male,female,all',
            'focus'       => 'required|in:training,motivation,learning',
    ...
    public function displayReviews($username) {
        $timeline = Timeline::where('username', $username)->with('event')->first();
        $event = $timeline->event;
        $reviews = Review::where('event_id', $event->id)->with('users')->Paginate(20);
        $theme = Theme::uses(Setting::get('current_theme', 'default'))->layout('default');
        $theme->setTitle(trans('common.invitemembers').' | '.Setting::get('site_title').' | '.Setting::get('site_tagline'));
        return $theme->scope('users/eventreview', compact('timeline', 'event', 'reviews'))->render();
    }
